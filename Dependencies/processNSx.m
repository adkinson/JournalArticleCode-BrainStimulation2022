    ...
    if ~isempty(intersect(NSx.MetaTags.Timestamp,syncSampleWindow)) && combineData
        ...
        if length(unique(samplesPerSet))>1
            ...
            if strcmp(trimOrPad,'Trim')
                ...
                switch trimOrPad
                    case 'Zero Pad'
                        tailPaddingFunctionHandle = @zeros;
                    case 'NaN Pad'
                        tailPaddingFunctionHandle = @nan;
                end
                for j = 1:length(minIdx)
                    tailPadding = tailPaddingFunctionHandle(size(NSx.Data{minIdx(j)},1),maxSam-samplesPerSet(minIdx(j)));
                    NSx.Data{minIdx(j)} = cat(2,NSx.Data{minIdx(j)},tailPadding);
                end
            end
        end
